...
app.post('/api/analyze-tool', async (req, res) => {
  try {
    ...
    // АКТУАЛЬНАЯ ВЕРСИЯ МОДЕЛИ (январь 2026)
    // Получите актуальную версию на: https://replicate.com/llava-13b
    const MODEL_VERSION = "55907c2c36e62783c96c012f6e2d5919a0d8b37769d8d7c5e5d5e5d5e5d5e5d5";
    ...
    if (error.message?.includes('413') || error.message?.includes('Payload too large')) {
      return res.status(413).json({ 
        error: 'Payload too large',
        message: 'Изображение слишком большое. Сожмите до 800 пикселей и 70% качества',
...
app.listen(PORT, () => {
  ...
  console.log('║  ⚠️ Лимит размера запроса: 10 МБ                           ║');
